...
const Folders = ({parent, folders, setParent,setFolders}) =>{
    ...
    const handleDelete=(fid)=>{
        ...
        if(folderToDelete && folderToDelete.parent && updatedFolders[folderToDelete.parent]){
            updatedFolders[folderToDelete.parent].child = updatedFolders[folderToDelete.parent].child.filter(
                (childId)=> childId !== fid
            )
        }
    ...
    return (
        <div className="mt-5 mx-24">
            <h1 className="text-slate-600 text-3xl">Folder Manager</h1>
             ...
             {/* back to home */}
                <button onClick={()=>{setParent(0)}} className="bg-blue-500 cursor-pointer text-white mr-5 hover:bg-blue-700">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6 ">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                    </svg>
                </button>

                <button className="bg-green-700 hover:bg-green-800 cursor-pointer flex text-white" onClick={handleCreateFolder}>
            ...
            {Object.keys(folders).map((fid)=>{
                ...
                return (
                    <div className="flex mt-5 justify-between items-center hover:shadow-md border border-1 rounded mx-5">
                        <span className="flex p-1 m-3 rounded cursor-pointer" onClick={()=>{setParent(fid)}}>

                            <svg xmlns="http://www.w3.org/2000/svg" fill="#f7cb52" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 mr-2">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 9.776c.112-.017.227-.026.344-.026h15.812c.117 0 .232.009.344.026m-16.5 0a2.25 2.25 0 0 0-1.883 2.542l.857 6a2.25 2.25 0 0 0 2.227 1.932H19.05a2.25 2.25 0 0 0 2.227-1.932l.857-6a2.25 2.25 0 0 0-1.883-2.542m-16.5 0V6A2.25 2.25 0 0 1 6 3.75h3.879a1.5 1.5 0 0 1 1.06.44l2.122 2.12a1.5 1.5 0 0 0 1.06.44H18A2.25 2.25 0 0 1 20.25 9v.776" />
                            </svg>

                            {thisFolder.title}   
                        </span>
                        <button className="text-red-500 bg-transparent hover:text-red-700 mr-4" onClick={()=>handleDelete(fid)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
